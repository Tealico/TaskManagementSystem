...
public class TaskRepository {
	...
	private static final String TASK_BY_GROUP_ID = "select task FROM TaskEntity task join task.user user join user.groups gr where gr.id=?1";
	...
	public List<TaskEntity> getAllTasksByUserId(long userId) {
		TypedQuery<TaskEntity> query = entityManager.createQuery(TASK_BY_USER_ID, TaskEntity.class).setParameter(1,
				userId);
		return query.getResultList();
	}

	public List<TaskEntity> getAllTasksByGroupId(long groupId) {
		TypedQuery<TaskEntity> query = entityManager.createQuery(TASK_BY_GROUP_ID, TaskEntity.class).setParameter(1,
				groupId);
	...
	public List<TaskEntity> getAllTasksByComplexity(String complexity) {
		TypedQuery<TaskEntity> query = entityManager.createQuery(TASK_BY_COMPLEXITY, TaskEntity.class).setParameter(1,
				complexity);
		return query.getResultList();
	}

	public List<TaskEntity> getAllTasksByStatus(String status) {
		TypedQuery<TaskEntity> query = entityManager.createQuery(TASK_BY_STATUS, TaskEntity.class).setParameter(1,
				status);
		return query.getResultList();
	}
	...
	public List<TaskEntity> getAllTasksByComplexityAndStatus(String status, String complexity) {
		TypedQuery<TaskEntity> query = entityManager.createQuery(TASK_BY_COMPLEXITY_AND_STATUS, TaskEntity.class)
				.setParameter(1, complexity).setParameter(2, status);
		return query.getResultList();
	}
	...
	public List<TaskEntity> getAllTasksByUserAndStatus(long userId, String status) {
		TypedQuery<TaskEntity> query = entityManager.createQuery(TASK_BY_USER_ID_AND_STATUS, TaskEntity.class)
				.setParameter(1, userId).setParameter(2, status);
		return query.getResultList();
	}
	...
	public List<TaskEntity> getAllTasksByUserAndComplexity(long userId, String complexity) {
		TypedQuery<TaskEntity> query = entityManager.createQuery(TASK_BY_USER_ID_AND_COMPLEXITY, TaskEntity.class)
				.setParameter(1, userId).setParameter(2, complexity);
		return query.getResultList();
	}
	...
	public List<TaskEntity> getAllTasksByUserIdAndComplexityAndStatus(long userId, String status, String complexity) {
		TypedQuery<TaskEntity> query = entityManager
				.createQuery(TASK_BY_USER_ID_AND_COMPLEXITY_AND_STATUS, TaskEntity.class).setParameter(1, userId)
				.setParameter(2, complexity).setParameter(3, status);
		return query.getResultList();
	}
}
